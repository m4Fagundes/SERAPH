...
4. Cellpose's own best (cellprob −0.5, F1 0.745) beats its default (0.0, F1 0.728) — a small,
   honest tuning gain. Its instance curve is non-monotonic at the low end (fewer instances at
   low cellprob). **CORRECTION (verified empirically):** this is NOT mask merging — it is the
   `flow_threshold` filter. At cellprob~0.1 Cellpose forms ~45 masks but the flow-consistency
   filter (flow_threshold=0.4) rejects ~23 of them (vs only ~3 at cellprob~0.4); with
   flow_threshold=0 the count jumps 22→45. Low cellprob admits faint/noisy pixels that ruin the
   flow consistency of weak nuclei → those masks are discarded (they vanish, they do NOT merge:
   measured 9 vanished, 0 merged on severe-01-roi1). The non-monotonic "hook" is a
   cellprob × flow_threshold interaction artifact.
...
### Q16 — Final comparison with every model TUNED (best config, full metrics, 100 ROIs)
**Setup:** each model re-run at its best-F1 config with full metrics (`tuned_comparison.py` +
`pathosam_tuned_fix.py` → `results_tuned.csv`, `results_pathosam_tuned.csv`). Configs: Cellpose
cellprob=logit(0.4); PathoSAM untiled fg 0.5; InstanSeg seed 0.8/peak 7/mask 0.5; CellViT fg 0.8.

| Model | Config | Precision | Recall | F1 | Dice | Bound.Recall | Bound.F | FP |
|---|---|---|---|---|---|---|---|---|
| **Cellpose** | thr 0.4 | 0.811 | 0.759 | **0.784** | 0.885 | 0.665 | 0.666 | 480 |
| **PathoSAM** | untiled, fg 0.5 | 0.561 | 0.796 | **0.658** | 0.872 | 0.653 | 0.598 | 1690 |
| **InstanSeg** | seed 0.8 | 0.675 | 0.611 | 0.641 | 0.868 | 0.522 | 0.537 | 798 |
| **CellViT-SAM** | fg 0.8 | 0.522 | 0.787 | 0.628 | 0.873 | 0.683 | 0.614 | 1953 |

**Default→Tuned F1 gain:** Cellpose +0.009, **PathoSAM +0.046 (untiling)**, InstanSeg +0.006, CellViT ~0.

**Conclusion — refined ranking claim:** Cellpose is **1st under every condition** (default, tuned,
both tissues, all thresholds) — robust. **But the 2nd–4th order changes with tuning:** default was
Cellpose>InstanSeg>CellViT>PathoSAM; tuned is **Cellpose>PathoSAM>InstanSeg>CellViT**. PathoSAM
jumps to 2nd once untiled (its default-config penalty was the tiling-on-small-ROI artifact, Q8).
Takeaway: comparing only at default configs is unfair to models with poor defaults — a tuned
(or at least sanity-checked) comparison matters. Cellpose's lead, however, is not config-dependent.
...
- **2026-06-01** — Q16: final TUNED comparison (all 4 at best config, full metrics). Cellpose
  0.784 (1st, robust), PathoSAM 0.658 (2nd, +0.046 from untiling), InstanSeg 0.641, CellViT
  0.628. Tuning reorders 2nd–4th (PathoSAM overtakes InstanSeg once untiled); Cellpose lead holds.
...
- **2026-06-01** — CORRECTION to Q12 Cellpose curve: the low-threshold instance drop is the
  `flow_threshold` filter rejecting low-confidence masks (cellprob 0.1: 45 masks formed, 22 kept
  with flow=0.4, 45 with flow=0), NOT merging (0 merges measured, 9 vanished). User-driven fix.
